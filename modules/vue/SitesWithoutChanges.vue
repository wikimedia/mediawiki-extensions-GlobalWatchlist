		...
		>
			<ul id="ext-globalwatchlist-vue-sites-without-changes-list">
				...
				>
					...
					<a :href="site['special-watchlist-url']" target="_blank">{{ site['site-name'] }}</a>
					(<a :href="site['special-edit-watchlist-url']" target="_blank">{{ site['edit-watchlist-msg'] }}</a>)
...
module.exports = {
	...
	computed: {
		emptysiterows: function () {
			const that = this;
			const rows = this.emptysitelist.map( ( site ) => ( {
